...
import { CompanyRegister } from '../model/company-register';
...
export class AuthService {
  companyRegiser(formData: FormData) {
    return this.http.post<AppResponse>(
      `${urlEndpoint.baseUrl}/auth/company/register`,
      formData
    );
  }
  ...
  isRecruiterInSubject = new BehaviorSubject<boolean>(false);

  isAdmin$: Observable<boolean> = this.isAdminSubject.asObservable();
  isLoggedIn$: Observable<boolean> = this.isLoggedInSubject.asObservable();
  isRecruiter$: Observable<boolean> = this.isRecruiterInSubject.asObservable();
  ...
  isRecruiter(): boolean {
    return this.isRecruiterInSubject.value;
  }
  setLoggedIn(user: AppUser): void {
    ...
    } else if (user.role === CONSTANT.RECRUITER) {
      this.isRecruiterInSubject.next(true);
      this.router.navigate(['/recruiter'], { replaceUrl: true });
    }
  }
  regiser(register: FormData) {
    return this.http.post<AppResponse>(
      `${urlEndpoint.baseUrl}/auth/create`,
      register
    );
  }
}
